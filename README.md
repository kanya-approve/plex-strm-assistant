...
straight for the media part as some clients do. The request goes out half a minute later, because
Plex reads the same source to analyse it and can otherwise leave the player waiting on it. Plex then
records the real container, codecs, bitrate and every audio and subtitle track. The request uses the server's own token from `Preferences.xml`, so
