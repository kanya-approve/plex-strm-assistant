...
straight for the media part as some clients do. Plex then records the real container, codecs,
bitrate and every audio and subtitle track. The request uses the server's own token from `Preferences.xml`, so
