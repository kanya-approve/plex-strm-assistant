...
Set `ANALYZE_ON_PLAY=true` and, the first time a client asks the gateway to play such an item
(in either gateway mode), the gateway asks Plex to analyse it. Plex then records the real container, codecs, bitrate and
