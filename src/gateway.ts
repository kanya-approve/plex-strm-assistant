...
function analyzeOnPlay(metadataId: string): void {
  ...
  // PMS reads the same source to analyse it, so let the play get going first.
  setTimeout(() => sendAnalyze(metadataId), ANALYZE_DELAY_MS).unref();
}

function sendAnalyze(metadataId: string): void {
  if (!isUnanalysedStrm(metadataId)) return;
